...
public:
    ...
    StereoSample render(const RuntimeParams& p)
    {
        ...
        const float motion = juce::jlimit(-1.0f, 1.0f, lfoValues[0] * (0.45f + 0.9f * p.motionAmount) + lfoValues[1] * 0.35f * p.motionAmount + env3v * 0.2f * p.motionAmount);
        const float pitchRatio = std::pow(2.0f, modPitch * 0.25f + motion * 0.03f);
        const float wobble = 1.0f + (p.wompAmount * 0.55f + p.motionAmount * 0.25f) * lfoValues[0];
        ...
        oscA.setPosition(p.oscAPos + modAPos * 0.35f + motion * (0.10f + 0.18f * p.motionAmount));
        oscB.setPosition(p.oscBPos + modBPos * 0.35f - motion * (0.12f + 0.20f * p.motionAmount));
        ...
        const float toneTilt = juce::jmap(p.toneAmount, 0.0f, 1.0f, 0.62f, 1.75f);
        const float cutoff = juce::jlimit(20.0f, 18000.0f, p.cutoff * toneTilt * std::pow(2.0f, modCutoff * 4.0f) * (1.0f + (p.wompAmount * 0.45f + p.motionAmount * 0.20f) * lfoValues[0]));
        ...
        {
            const float hornL = std::sin(l * (4.0f + 12.0f * p.warhornAmount) + env2v * 3.0f + motion * 2.0f) * 0.35f;
            const float hornR = std::sin(r * (4.0f + 12.0f * p.warhornAmount) + env2v * 3.0f - motion * 2.0f) * 0.35f;
        ...
        const float distDrive = p.distDrive + modDist * 0.35f + p.warhornAmount * 0.25f + p.fmAmount * 0.2f + p.growlAmount * 0.35f + p.toneAmount * 0.10f;
        ...
        l += std::tanh(bodyMono * 1.8f) * (0.32f * p.bodyAmount);
        r += std::tanh(bodyMono * 1.8f) * (0.32f * p.bodyAmount);
