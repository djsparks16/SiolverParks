...
struct RuntimeParams
{
    ...
    float bodyAmount = 0.0f, growlAmount = 0.0f, shimmerAmount = 0.0f, motionAmount = 0.0f;
    ...
    float reverbSize = 0.55f, reverbTone = 0.55f, chorusRate = 0.4f, delayTime = 0.35f;
...
public:
    ...
    StereoSample render(const RuntimeParams& p)
    {
        ...
        const float wobble = 1.0f + p.wompAmount * 0.55f * lfoValues[0];
        const float hzA = currentHz * std::pow(2.0f, (float) (12 * p.oscAOct + p.oscASemi) / 12.0f) * std::pow(2.0f, p.oscAFine / 1200.0f) * pitchRatio * wobble;
        const float hzB = currentHz * std::pow(2.0f, (float) (12 * p.oscBOct + p.oscBSemi) / 12.0f) * std::pow(2.0f, p.oscBFine / 1200.0f) * pitchRatio * (1.0f - p.reeseAmount * 0.035f * lfoValues[1]);
        ...
        oscA.setPosition(juce::jlimit(0.0f, 1.0f, p.oscAPos + modAPos * 0.35f + motion * 0.18f));
        oscB.setPosition(juce::jlimit(0.0f, 1.0f, p.oscBPos + modBPos * 0.35f - motion * 0.15f));
        ...
        const auto oscOutA = renderStack(oscA, extraA, p.oscAUnison + p.comboAmount * 2.0f, p.oscADetune + p.reeseAmount * 0.25f, p.oscABlend, juce::jlimit(0.0f, 1.0f, p.oscAPan + modPan * 0.2f - p.reeseAmount * 0.15f), true);
        const auto oscOutB = renderStack(oscB, extraB, p.oscBUnison + p.comboAmount * 2.0f, p.oscBDetune + p.reeseAmount * 0.25f, p.oscBBlend, juce::jlimit(0.0f, 1.0f, p.oscBPan - modPan * 0.2f + p.reeseAmount * 0.15f), false);
        ...
        const float body = std::tanh(comboMono * (1.3f + p.bodyAmount * 3.8f));
        l += body * (0.18f + 0.32f * p.bodyAmount);
        r += body * (0.18f + 0.32f * p.bodyAmount);

        if (! p.subDirect) { l += subSample; r += subSample; }
        if (! p.noiseDirect) { l += noiseRaw * noisePanL; r += noiseRaw * noisePanR; }

        const float cutoff = juce::jlimit(20.0f, 18000.0f, p.cutoff * std::pow(2.0f, modCutoff * 4.0f) * (1.0f + p.wompAmount * 0.45f * lfoValues[0]));
        ...
        {
            const float growlMod = (0.4f * oscA.process(0.13f) - 0.35f * oscB.process(-0.09f));
            l = std::tanh(l + growlMod * (0.16f + 0.44f * p.growlAmount));
            r = std::tanh(r - growlMod * (0.16f + 0.44f * p.growlAmount));
        }

        if (p.warhornAmount > 0.001f)
        {
            const float hornL = std::sin(l * (4.0f + 12.0f * p.warhornAmount) + env2v * 3.0f) * 0.35f;
            const float hornR = std::sin(r * (4.0f + 12.0f * p.warhornAmount) + env2v * 3.0f) * 0.35f;
            l = std::tanh(l + hornL + p.fmAmount * oscB.process(0.17f) * 0.15f);
            r = std::tanh(r + hornR + p.fmAmount * oscA.process(-0.11f) * 0.15f);
        }

        if (p.subDirect) { l += subSample; r += subSample; }
        if (p.noiseDirect) { l += noiseRaw * noisePanL; r += noiseRaw * noisePanR; }

        const float distDrive = p.distDrive + modDist * 0.35f + p.warhornAmount * 0.25f + p.fmAmount * 0.2f;
        ...
        const float side = 0.5f * (l - r) * (0.4f + p.stereoWidth * 1.2f + p.reeseAmount * 0.5f);
        l = mid + side;
        r = mid - side;
        const float air = p.airAmount * 0.12f;
        l = std::tanh(l + (l - mid) * air);
        r = std::tanh(r + (r - mid) * air);
        if (p.shimmerAmount > 0.001f)
        {
            const float brightL = std::tanh((l - comboMono) * (1.5f + p.shimmerAmount * 3.0f));
            const float brightR = std::tanh((r - comboMono) * (1.5f + p.shimmerAmount * 3.0f));
            l += brightL * (0.05f + 0.18f * p.shimmerAmount);
            r += brightR * (0.05f + 0.18f * p.shimmerAmount);
        }
