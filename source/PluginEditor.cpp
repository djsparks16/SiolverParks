...
namespace
{
    ...
    Colour bg()        { return Colour(0xff060914); }
    Colour panel()     { return Colour(0xff111827); }
    Colour panel2()    { return Colour(0xff1a1f3a); }
    Colour edge()      { return Colour(0x66c467ff); }
    Colour accent()    { return Colour(0xff66f2ff); }
    Colour accent2()   { return Colour(0xffff66d9); }
    ...
    : AudioProcessorEditor(&p), audioProcessor(p), scope(p), spectrum(p), lfoShape(p), matrixView(p)
{
    ...
    subtitle.setText("Level 9000 rewrite · bass monster engine · oscillators · routing · FX", dontSendNotification);
...
void FiveParksVST3AudioProcessorEditor::resized()
{
    ...
    bassBox.setBounds(middle.removeFromLeft(420));
    middle.removeFromLeft(8);
