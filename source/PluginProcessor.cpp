...
juce::AudioProcessorValueTreeState::ParameterLayout FiveParksVST3AudioProcessor::createParameters()
{
    ...
    p.push_back(pf("body_amt", "Body", 0.0f, 1.0f, 0.30f));
    p.push_back(pf("growl_amt", "Growl", 0.0f, 1.0f, 0.18f));
    p.push_back(pf("shimmer_amt", "Shimmer", 0.0f, 1.0f, 0.12f));
    p.push_back(pf("motion_amt", "Motion", 0.0f, 1.0f, 0.22f));
    ...
    p.push_back(pf("reverb_size", "Reverb Size", 0.0f, 1.0f, 0.58f));
    p.push_back(pf("reverb_tone", "Reverb Tone", 0.0f, 1.0f, 0.55f));
    p.push_back(pf("chorus_rate", "Chorus Rate", 0.0f, 1.0f, 0.35f));
    p.push_back(pf("delay_time", "Delay Time", 0.0f, 1.0f, 0.35f));
...
void FiveParksVST3AudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi)
{
    ...
    p.reverbSize = getF(apvts, "reverb_size"); p.reverbTone = getF(apvts, "reverb_tone"); p.chorusRate = getF(apvts, "chorus_rate"); p.delayTime = getF(apvts, "delay_time");
    ...
    p.bodyAmount = getF(apvts, "body_amt"); p.growlAmount = getF(apvts, "growl_amt"); p.shimmerAmount = getF(apvts, "shimmer_amt"); p.motionAmount = getF(apvts, "motion_amt");
    ...
    for (int sample = 0; sample < numSamples; ++sample)
    {
        ...
        chorusPhase += (0.08f + p.chorusRate * 0.95f + p.reeseAmount * 0.12f + p.motionAmount * 0.3f) / (float) currentSampleRate;
        if (chorusPhase >= 1.0f) chorusPhase -= 1.0f;
        const float chorusMod = 0.0022f + (0.0018f + p.stereoWidth * 0.0024f + p.motionAmount * 0.0015f) * std::sin(chorusPhase * juce::MathConstants<float>::twoPi);
        ...
        const int delaySamps = (int) ((0.12 + 0.50f * p.delayTime + 0.08f * p.wompAmount) * currentSampleRate);
    ...
    reverbParams.roomSize = 0.18f + 0.72f * p.reverbSize;
    reverbParams.damping = 0.12f + 0.76f * (1.0f - p.reverbTone);
